...
import 'package:elearning/services/examApi.dart';
import 'package:elearning/ui/pages/result_Page.dart';
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'Total Questions: ${widget.quiz.questions.length}\nTotal Answered: ${userAnswers.where((answer) => answer != null).length}\nAre you sure you want to submit?'),
          actions: [
            if (!isTimeOverDialogShown) // Check if the time is not over
              TextButton(
                onPressed: () {
                  Navigator.of(context).pop(); // Close the dialog
                },
  ...
  void switchButtonAction(ButtonAction action) {
    switch (action) {
      case ButtonAction.Previous:
        if (isLastQuestion) {
          // If on the last question, revert to "Next Question" button
          setState(() {
            isLastQuestion = false;
          });
        }
...
enum ButtonAction { Previous, Next }
